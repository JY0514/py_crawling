        ...
        for slide in ann_cont_list:
            ...
            for tit_wrap in tit_wrap_list:
                ...
                if count == 0:
                    ...
                    # 존재 데이터
                    check_sql = (
                        f"SELECT count(*) FROM localdb.crawling WHERE 지원사업명 = '{name}' AND 마감일 = '{d_date}' AND 관련부처 = '{re_min}' AND 수행기관 = '{agency}' ")
                    cur.execute(check_sql)
                    result = cur.fetchone()
                    count_data = result[0]
...
# 비즈인포
def insert2(conn):
    url2 = "http://www.bizinfo.go.kr/web/lay1/bbs/S1T122C128/AS/74/list.do"
